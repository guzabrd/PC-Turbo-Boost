...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('pc_turbo_history');
    if (saved) {
      setHistory(JSON.parse(saved).map((h: any) => ({ ...h, date: new Date(h.date) })));
    } else {
      const initial: Optimization[] = [
        { id: '1', title: 'Limpeza de Arquivos Temporários', type: 'CMD', date: new Date(), status: 'applied' },
        { id: '2', title: 'Otimização de Memória RAM', type: 'PowerShell', date: new Date(Date.now() - 86400000), status: 'applied' }
      ];
      setHistory(initial);
      localStorage.setItem('pc_turbo_history', JSON.stringify(initial));
    }
  }, []);

  const addToHistory = (opt: Optimization) => {
    const newHistory = [opt, ...history];
    setHistory(newHistory);
    localStorage.setItem('pc_turbo_history', JSON.stringify(newHistory));
  };

  const navItems = [
    { id: View.CHAT, label: 'Turbo Assistant', icon: MessageSquare },
    { id: View.DIAGNOSTIC, label: 'Smart Diagnostic', icon: Activity },
    { id: View.SCRIPTS, label: 'Boost Scripts', icon: Terminal },
    { id: View.HISTORY, label: 'Optimization Log', icon: HistoryIcon },
  ];
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 mt-auto">
            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50">
              ...
              <p className="text-xs text-slate-400 mb-3">Your PC is protected and ready for more FPS.</p>
